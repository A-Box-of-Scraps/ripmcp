...
preparing = runtime == 'npm' or '--package' in args or ('--from' in args and '-c' in args) or args[0] in ('pull', 'image')
if preparing:
    ...
    if (root / 'stall').exists():
        child = subprocess.Popen([sys.executable, '-c', 'import time; time.sleep(1000)'], close_fds=False)
