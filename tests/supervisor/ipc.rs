...
pub(super) fn shutdown_best_effort(fixture: &Fixture) {
    ...
    let request: Value = json!({"protocol": 1, "build": env!("CARGO_PKG_VERSION"), "nonce": record["nonce"], "context": record["context"]});
...
fn hello(fixture: &Fixture) -> Value {
    ...
    json!({"protocol": 1, "build": env!("CARGO_PKG_VERSION"), "nonce": record["nonce"], "context": record["context"]})
}
