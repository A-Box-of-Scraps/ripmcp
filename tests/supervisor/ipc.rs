...
pub(super) fn shutdown_best_effort(fixture: &Fixture) {
    ...
    let request: Value = json!({"protocol": 2, "build": env!("CARGO_PKG_VERSION"), "nonce": record["nonce"], "context": record["context"]});
...
fn hello(fixture: &Fixture) -> Value {
    ...
    json!({"protocol": 2, "build": env!("CARGO_PKG_VERSION"), "nonce": record["nonce"], "context": record["context"]})
}
