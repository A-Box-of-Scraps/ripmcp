...
By the end, you will have registered one remote server, inspected a tool, and saved
one invocation result. Discovery does not invoke tools.

## Before you start

You need:

- A Linux shell with [ripmcp installed](../how-to/install.md).
...
start with the [offline tutorial](inspect-json.md) instead.
...
[authentication setup](../how-to/authenticate.md) now. Otherwise continue.
...
On success, `result.json` contains the complete MCP tool-result envelope, not just
text content. Exit code 7 means a tool-reported failure whose result is still saved.
...
[offline tutorial](inspect-json.md) to practice selecting values with `jq`.

## 5. Remove the tutorial registration
...
Next: [manage tools and local processes](../how-to/tools.md), or
[use project scope](../how-to/projects.md).
